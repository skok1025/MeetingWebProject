...
/**
 * 회원가입시, 진행하는 센스 테스트 관련 Servlet 클래스
 * @author 김석현
 *
 */
@WebServlet("/main/sensetest.do")
public class SenseTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for(int i=1; i<=10; i++) {
	         result = req.getParameter("hq"+i).replaceAll(" ", "");
	         answer = req.getParameter("answer"+i).replaceAll(" ", "");
		...
		System.out.println("Humor:"+scorecnt);
		
		// SenseTest 가져오기
		JoinDAO dao = new JoinDAO();
		 ArrayList<ExamDTO> list = dao.getTestList("Sense");
		...
		req.setAttribute("humor", scorecnt);
		req.setAttribute("gender", gender);
		
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/view/main/sensetest.jsp");
